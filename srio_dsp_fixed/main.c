...
main()
{
	...
	for (i = 0, n = 0; i < SRIO_TESTTIMES; i++) 
	{ 

  		for (j = 0, pBUF= (int*)SRIO_WRITEBUF; j < SRIO_TESTLEN; j++, *(pBUF++) = ((j+i) + ((j+i) << 16)) );

		srio_write(SRIO_WRITEBUF, SRIO_TARGETBUF_ADDR, SRIO_TESTLEN);  //write to FPGA

		for (k = 1; k < 1000000; k++) ;

		srio_read(SRIO_TARGETBUF_ADDR, SRIO_READBUF, SRIO_TESTLEN);    //read from FPGA
		
		for (k = 1; k < 1000000; k++) ;	

 	  	if (memcmp((void *)SRIO_WRITEBUF, (void *)SRIO_READBUF, SRIO_TESTLEN) != 0)    //compare data from FPGA with the original data
			n++;
	}
	if(n != 0)
		printf("\n\tFAIL: SRIO transfers fails %d times\n",n);
	else
		printf("\n\tSUCESS: SRIO transfer test passed! \n");
...
void SRIO_init()                    //initialize C6455 SRIO
{
	...
	SRIO_PER_SET_CNTL = 0x0500026F; // bootcmpl=1 2012-12-25
